(function() {
  ...
  function parseCommitContents(buffer) {
    ...
    buf_util.getBufferFromFile(fileName, function(error, compressedBuffer) {
      ...
      zlib.inflate(compressedBuffer, function(err, buffer) {
        ...
        if (info.type === 'tree') {
          ...
          // we don't parse blob
        } else if (info.type === 'commit') {
          info.content = parseCommitContents(br.slice());
        }
